...
test_that('Same output when seed set and input the same', {
  ...
  set.seed(1)
  norm1 = HiCcompare::hic_loess(tab1, check.differences = T)
  set.seed(1)
  norm2 = HiCcompare::hic_loess(tab2, check.differences = T)
...
test_that('Pipeline works', {
  ...
  set.seed(1)
  norm1 = HiCcompare::hic_loess(tab, check.differences = F)
  norm2 = HiCcompare::hic_diff(norm1)
  norm3 = HiCcompare::hic_diff(norm1, diff.thresh = 1, Plot = F)
  norm4 = HiCcompare::hic_diff(norm1, diff.thresh = 0.6, Plot =F)
  norm5 = HiCcompare::hic_diff(norm1, diff.thresh = NA, Plot = F)
