...
.kal <- function(hic.table) {
  ...
  MD.plot2(new.table$adj.M, new.table$D, p.adjust(new.table$p.value, 'fdr'))
  return(new.table)
} 


.fisher <- function(hic.table) {
  # get sum of all IFs at each distance
  N1 <- aggregate(hic.table$adj.IF1, by = list(hic.table$D), sum)
  N2 <- aggregate(hic.table$adj.IF2, by = list(hic.table$D), sum)
  colnames(N1) <- c('D', 'N1')
  ...
  IF_mat <- cbind(new.table$adj.IF1, new.table$adj.IF2, new.table$N1, new.table$N2) %>% as.matrix()
...
.get_fisher <- function(x) {
  m <- matrix(c(x[1], x[3] - x[1], x[2], x[4] - x[2]), ncol = 2)
...
```{r}
hic.table <- backup.table
hic.table <- make_changes(hic.table, FC = 3)
