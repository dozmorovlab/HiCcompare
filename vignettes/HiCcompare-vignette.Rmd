...
To determine how to filter your data you can use the `filter_params()` function. This will produce a plot of the Matthews Correlation Coefficient (MCC) vs. the A quantile filtered out or the A minimum value filtered out. This plot should help you determine a reasonable value to set your filtering parameter in `hic_compare`. You may need to adjust the `numChanges` option and the `FC` option depending on the resolution of the data and how noisy it is.

Here we check the filtering by A quantile:

```{r, message=FALSE}
filter_params(hic.table, A.quantile = TRUE)
```

And here by A minimum value:

```{r, message=FALSE}
filter_params(hic.table, A.quantile = FALSE)
```


Difference detection can be performed using the `hic_compare()` function. Before difference detection you will need to decide how you would like to filter out interactions with low average expression. You can either filter based on the quantile of A or by a specific value of A. Quantile filtering is recommended for lower resolution data (1MB or 500KB). Picking a minimum value of A would like to filter is recommend for higher resolution data (<= 100KB). Once you have a normalized `hic.table` or list of `hic.tables` detect differences with an A quantile filter as follows:

```{r}
hic.table <- hic_compare(hic.table, A.quantile = 0.1, adjust.dist = TRUE, p.method = 'fdr', Plot = TRUE)
```

Or with a minimum A value as follows:

```{r}
hic.table <- hic_compare(hic.table, A.min = 15, adjust.dist = TRUE, p.method = 'fdr', Plot = FALSE)
```
