...
```{r}
# load data
githubURL <- "https://github.com/dozmorovlab/HiCcompare/raw/supplemental/Supplemental_data/S1_File_data.RData"
load(url(githubURL))
```
...
```{r}
diffs = hic_loess(unscaled.tab,  Plot = T, check.differences = F, diff.thresh = NA, Plot.smooth = FALSE)
```
...
```{r}
diffs = hic_loess(chr11.tab, Plot=T, check.differences = F, Plot.smooth = FALSE)
```


```{r}
diffs = hic_diff(diffs, diff.thresh = 1, Plot=T, Plot.smooth = FALSE)
tab = diffs
...
```{r}
diffs = hic_diff(chromoR.tab, Plot = T, diff.thresh = 1, Plot.smooth = FALSE)
counts = sum(diffs$p.value < 0.05)
print(paste0(counts, ' differences found between the datasets'))
```
...
```{r}
diffs = hic_diff(ice.table, Plot = T, diff.thresh = 1, Plot.smooth = FALSE)
counts = sum(diffs$p.value < 0.05)
print(paste0(counts, ' differences found between the datasets'))
```
...
```{r}
diffs = hic_diff(kr.table, Plot = T, diff.thresh = 1, Plot.smooth = FALSE)
counts = sum(diffs$p.value < 0.05)
print(paste0(counts, ' differences found between the datasets'))
```
...
```{r}
diffs = hic_diff(scn.table, Plot = T, diff.thresh = 1, Plot.smooth = FALSE)
counts = sum(diffs$p.value < 0.05)
print(paste0(counts, ' differences found between the datasets'))
```
...
```{r}
loess.diffs = hic_loess(chr11.unscaled.tab, check.differences = F, Plot = T , Plot.smooth = FALSE)
```


```{r}
loess.diffs = hic_diff(loess.diffs, Plot = T, diff.thresh = 1, Plot.smooth = FALSE)
