...
The tables show that the most true differences are detected after joint `loess` normalization compared to the other normalization techniques. `loess` also has the lowest number of false positives among the normalization techniques. With `loess`, differences at smaller fold changes (1.5 and 2) are able to be detected more reliably compared to the other methods and this superiority continues to the higher fold changes though the individual normalization methods tend to make up some ground.`MA` normalization performed almost as well as `loess` at lower fold changes and was about equivalent at the higher fold changes. `ChromoR` performed the worst of the normalization techniques tested while `KR` and `SCN` tended to perform better. 
